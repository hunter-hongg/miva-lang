...
static int mvp_run_tests(MvpTest tests[], int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        try {
            ...
            if (res == 0) {
                std::printf("%sPASS%s %s (%ldms)\n", MVP_COLOR_GREEN, MVP_COLOR_RESET, tests[i].name, ms);
                passed++;
            } else {
            ...
            auto end = std::chrono::steady_clock::now();
            auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
            std::printf("%sFAIL%s %s: %s (%ldms)\n", MVP_COLOR_RED, MVP_COLOR_RESET, tests[i].name, e.what(), ms);
            failed++;
        }
    }
    ...
    std::printf("--------------------\n");
    if (failed == 0) {
        std::printf("%sAll tests passed.%s (%d/%d)\n", MVP_COLOR_GREEN, MVP_COLOR_RESET, passed, count);
        return 0;
    } else {
        std::printf("%s%d/%d tests failed.%s\n", MVP_COLOR_RED, failed, count, MVP_COLOR_RESET);
        return 1;
    }
}
